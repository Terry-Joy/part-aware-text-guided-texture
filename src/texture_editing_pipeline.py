...
from .syncmvd.step2 import step_tex
from .utils import *
...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
    ...
    def initialize_pipeline(
        ...
        # if painted_views is None and "images" in locals() and images is not None:
        print('painted_views', painted_views)
        if painted_views is not None:
            # 1. 把 VAE 搬到 GPU
            # self.vae.to(self._execution_device)
            
            # 2. 直接调用函数（它内部自带了归一化和 0.18215 缩放）
            # 注意：传入的 images 必须是 [0, 1] 范围的张量
            _images_path = painted_views 
            _mask_path = painted_mask

            # 重新定义局部变量，避免覆盖路径参数，方便后面传参
            p_views_latent = None
            p_mask_latent = None
            # print('1114141414')
            # print(isinstance(_images_path, str) and os.path.exists(_images_path))
            if isinstance(_images_path, str) and os.path.exists(_images_path):
                from PIL import Image
                import torchvision.transforms.functional as TF

                # 1. 加载 6*1024 的长图并转 Tensor [1, 3, 1024, 6144]
                pil_img = Image.open(_images_path).convert("RGB")
                img_tensor = TF.to_tensor(pil_img).unsqueeze(0).to(self.text_encoder.dtype) # [1, 3, 1024, 6144]
                ...
                print('have it')
                # self.vae.to("cpu") # 编完立刻滚回 CPU

            if isinstance(_mask_path, str) and os.path.exists(_mask_path):
                pil_mask = Image.open(_mask_path).convert("L")
                # 假设 Mask 也是 6 张横拼的，即 [1, 1, 768, 4608]
                mask_tensor = TF.to_tensor(pil_mask).unsqueeze(0).to(self.text_encoder.dtype)

                # 4. 如果是长图就切片，如果本来就是单张 768 就跳过
                if mask_tensor.shape[-1] > 768:
                    mask_list = torch.chunk(mask_tensor, 6, dim=-1)
                ...
                p_mask_latent = F.interpolate(
                    mask_batched.to(self._execution_device), 
                    size=(latent_h, latent_w), 
                    mode="nearest"
                )

            # 将处理好的 Latent 赋值回变量，传给 step_tex
            self.painted_views = p_views_latent # 现在是 [6, 4, 96, 96]
            self.painted_mask = p_mask_latent   # 现在是 [6, 1, 96, 96]
        # --- 构造结束 ---
...
#             print(f">>> 已保存 T=1000 初始雪花图至: {self.intermediate_dir}/step_init_T1000.jpg")
            ...
            for i, t in enumerate(timesteps):
                ...
                # if t > (1-multiview_diffusion_end)*num_timesteps:
                    ...
                    step_results = step_tex(
                        ...
                        exp=current_exp,
                        **extra_step_kwargs,
                        painted_views=self.painted_views,
                        painted_mask=self.painted_mask,
                    )
        ...
        if do_highres_fix and hasattr(self, 'original_highres_rgb'):
            ...
            orig_views = self.original_highres_rgb.to(self._execution_device)
            orig_mask = self.original_highres_mask.to(self._execution_device)
            
            # 1. 确保生成的图和原图（768）尺寸一致
            if gen_views.shape[-2:] != orig_views.shape[-2:]:
                gen_views = F.interpolate(gen_views.float(), size=orig_views.shape[-2:], mode="bilinear", align_corners=False).to(gen_views.dtype)
            
            # 2. Mask 羽化平滑接缝
            smooth_mask = T.functional.gaussian_blur(orig_mask, kernel_size=[highres_blur_kernel, highres_blur_kernel])
            
            # 3. 融合出修改后的图片 (此时尺寸是 768)
            fixed_views_tensor = orig_views * smooth_mask + gen_views * (1.0 - smooth_mask)
            
            # 🚀🚀🚀 4. 关键修复：送给烘焙器之前，必须拉伸到 uvp_rgb 要求的 1024 尺寸！
            target_size = (self.uvp_rgb.render_size, self.uvp_rgb.render_size) # 提取 1024
            fixed_views_tensor = F.interpolate(
                fixed_views_tensor.float(), 
                size=target_size, 
                mode="bilinear", 
                align_corners=False
            ).to(fixed_views_tensor.dtype)
