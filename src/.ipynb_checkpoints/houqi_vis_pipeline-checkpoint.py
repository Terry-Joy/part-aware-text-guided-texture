...
def apply_global_style_injection(latents, part_mask, eps=1e-6, adain_first=1):
    ...
    for pid in unique_ids:
        ...
        # 🌟 记录全局统计量
        step_stats[pid.item()] = {
            "mu_global": mu_global.detach().cpu(),
            "sigma_global": sigma_global.detach().cpu(),
            "views": {}
        }
        ...
        for v in range(latents.shape[0]):
            ...
            # 🌟 记录局部统计量
            step_stats[pid.item()]["views"][v] = {
                "mu_local": mu_local.detach().cpu(),
                "sigma_local": sigma_local.detach().cpu()
            }
    ...
    return aligned_latents, step_stats
...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
    ...
    def __call__(
        ...
        # 🌟 初始化分析字典并存入 part_mask
        self.cpna_analysis_data = {
            "part_mask": self.global_mask_96.detach().cpu(),
            "z_T_raw": latents.clone().detach().cpu(), # 对齐前的原始噪声
            "trajectory": {},
            "init_stats": None
        }
        ...
        if adain_first != 0:
            ...
            latents, init_stats = apply_global_style_injection(latents, self.global_mask_96, adain_first)
            self.cpna_analysis_data["z_T_aligned"] = latents.clone().detach().cpu()
            self.cpna_analysis_data["init_stats"] = init_stats
...
#             print(f">>> 已保存 T=1000 初始雪花图至: {self.intermediate_dir}/step_init_T1000.jpg")
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if t > (1-multiview_diffusion_end)*num_timesteps:
                    ...
                    # 存zt和z_{0|t}
                    latent_cpu_tuple = intermediate_results[-1]
                    self.cpna_analysis_data["trajectory"][t.item()] = {
                        "z_t": latent_cpu_tuple[0].clone(),
                        "z_0_pred": latent_cpu_tuple[1].clone()
                    }
                ...
                if i % log_interval == log_interval-1 or t == 1 or  i % 1 == 0:
        ...
        # ================= [新增 3]：实验结束，保存论文弹药库 =================    
        save_path = f"{self.intermediate_dir}/cpna_analysis_data.pt"
        torch.save(self.cpna_analysis_data, save_path)
        print(f"✅ 包含每一时刻 z_t、z_0_pred 以及熵减统计量的数据包已保存至: {save_path}")
