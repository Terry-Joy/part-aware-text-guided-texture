...
from .syncmvd.attention8 import SamplewiseAttnProcessor2_0, replace_attention_processors
...
# 如果在文件外部没有 import UVP，记得在这里或者文件顶部 import
# from src.renderer.project import UVProjection as UVP

def search_dynamic_cameras_submodular(
    mesh_path, 
    mesh_transform=None,
    num_cameras=8, 
    texture_size=1024, 
    render_size=512, 
    alpha=0.5, # 👈 唯一的参数：边际衰减因子 (替代了 w1, w2, w3)
    device="cuda:0"
):
    print(f"🚀 [Submodular Camera] 正在初始化 32 个候选相机并计算全局可见性 (alpha={alpha})...")
    
    # 1. 生成候选池 (Candidate Pool)：高低错落的 32 个视角
    elevations = [15, 30, 45, 60] # 4个维度的高低
    azimuths = [i * 45 for i in range(8)] # 8个方位的环绕
    candidate_poses = [(e, a) for e in elevations for a in azimuths]
    
    # 2. 借用你的 UVP 渲染出所有候选相机的 cos_maps
    uvp = UVP(texture_size=texture_size, render_size=render_size, 
              sampling_mode="nearest", channels=3, device=device)
    scale_factor = mesh_transform["scale"] if mesh_transform else 1
    
    if mesh_path.lower().endswith(".obj"):
        uvp.load_mesh(mesh_path, scale_factor=scale_factor)
    elif mesh_path.lower().endswith(".glb"):
        uvp.load_glb_mesh(mesh_path, scale_factor=scale_factor)
        
    uvp.set_cameras_and_render_settings(candidate_poses, centers=None, camera_distance=2.0)
    _, _, _, cos_maps, _, _ = uvp.render_geometry()
    
    # 强制取最后一个维度的第 0 层 (Top-1 Face，即最靠近相机的那层表面)
    if cos_maps.dim() == 4:
        cos_maps = cos_maps[..., 0] 
        
    H, W = cos_maps.shape[1], cos_maps.shape[2]
    
    # 3. 次模贪心搜索 (Submodular Greedy Search)
    selected_indices = []
    # 记录每个 UV 像素被观测的次数 N(x)
    obs_count = torch.zeros((H, W), dtype=torch.int32, device=device)
    
    # 用于记录画图数据 (但不作为返回值，以免打破原有 Pipeline)
    history_coverage = []
    history_consensus = []
    
    print(f"🎯 [Dynamic Camera] 开始基于次模衰减的贪心搜索 (目标: {num_cameras} 视角)...")
    for step in range(num_cameras):
        ...
        for i in range(len(candidate_poses)):
            ...
            candidate_cos = cos_maps[i]
            valid_mask = candidate_cos > 0.05 # 剔除极度边缘的扫视
            
            # ==========================================
            # 💡 核心：无参边际效用衰减公式
            # ==========================================
            current_N = obs_count[valid_mask].float()
            marginal_gain = candidate_cos[valid_mask] * (alpha ** current_N)
            
            score = marginal_gain.sum().item()
        ...
        # 更新全场的观测次数
        new_visible = cos_maps[best_idx] > 0.05
        obs_count[new_visible] += 1
        
        # 记录 log
        cov_ratio = (obs_count > 0).float().mean().item() * 100
        con_ratio = (obs_count > 1).float().mean().item() * 100
        history_coverage.append(cov_ratio)
        history_consensus.append(con_ratio)
        
        pose = candidate_poses[best_idx]
        print(f"   -> 第 {step+1} 选定: 仰角 {pose[0]}°, 方位角 {pose[1]}° | 得分: {best_score:.1f} | 覆盖率: {cov_ratio:.1f}%")
        
    final_poses = [candidate_poses[i] for i in selected_indices]
    
    print(f"✅ [Dynamic Camera] 搜索完成！UV有效覆盖率: {history_coverage[-1]:.2f}%, 达成共识率(>=2视角): {history_consensus[-1]:.2f}%")
    
    # 💡 如果你需要画图，可以把这两行数据保存到本地文件，这样就不需要改 return 签名了
    # np.save("history_coverage.npy", history_coverage)
    # np.save("history_consensus.npy", history_consensus)
    
    # 释放显存
    del uvp, cos_maps, obs_count
    torch.cuda.empty_cache()
    
    # 👈 保持你原本的单输出，完美兼容！
    return final_poses

def search_dynamic_cameras(
    mesh_path, 
    mesh_transform=None,
    num_cameras=8, 
    texture_size=1024, 
    render_size=512, 
    w1=0.3, # 边际覆盖权重 (找新洞)
    w2=0.5, # 共识强化权重 (找只看了一次的缝隙)
    w3=0.2, # 质量权重 (追求正面直拍)
    device="cuda:0"
):
    print("🚀 [Dynamic Camera] 正在初始化 32 个候选相机并计算全局可见性...")
    ...
    print(f"🎯 [Dynamic Camera] 开始共识驱动的贪心搜索 (目标: {num_cameras} 视角)...")
    for step in range(num_cameras):
        best_score = -1.0
        best_idx = -1
        ...
        for i in range(len(candidate_poses)):
            ...
            # W1: 边际覆盖收益 (看之前 0 次观测的地方)
            gain_cov = candidate_cos[valid_mask & (obs_count == 0)].sum()
            
            # W2: 共识强化收益 (看之前刚好 1 次观测的地方，打破独裁)
            gain_con = candidate_cos[valid_mask & (obs_count == 1)].sum()
            
            # W3: 整体观测质量 (只管是不是正拍)
            quality = candidate_cos[valid_mask].sum()
            
            # 综合得分
            score = w1 * gain_cov + w2 * gain_con + w3 * quality
        ...
        # 更新全场的观测次数
        new_visible = cos_maps[best_idx] > 0.5
        obs_count[new_visible] += 1
        
        pose = candidate_poses[best_idx]
        print(f"   -> 第 {step+1} 选定: 仰角 {pose[0]}°, 方位角 {pose[1]}° | 得分: {best_score.item():.2f}")
        
    final_poses = [candidate_poses[i] for i in selected_indices]
    
    # 算一下最终的覆盖率
    coverage_ratio = (obs_count > 0).float().mean().item()
    consensus_ratio = (obs_count > 1).float().mean().item()
    print(f"✅ [Dynamic Camera] 搜索完成！UV有效覆盖率: {coverage_ratio*100:.2f}%, 达成共识率(>=2视角): {consensus_ratio*100:.2f}%")
...
def apply_global_style_injection(latents, part_mask, eps=1e-6, adain_first=1):
    """
...
class StableSyncMVDPipeline(StableDiffusionControlNetPipeline):
    ...
    def initialize_pipeline(
        ...
        # if dongtai_camera:
        #     print("📸 检测到动态相机策略 (Dynamic Camera Poses) 输入！")
        #     self.camera_poses = camera_azims
        #     print('dongtai_camera is True', self.camera_poses)
        #     cam_count = len(self.camera_poses)
        #     # 对于动态相机，我们可以让每个相机 attend 到前一个和后一个（形成环状共识），
        #     # 也可以让他们 attend 到全场（因为只有8个，全连通代价也不大）
        #     for i in range(cam_count):
        #         self.attention_mask.append([(i-1)%cam_count, i, (i+1)%cam_count])
        if dongtai_camera:
            self.camera_poses = camera_azims 
            cam_count = len(self.camera_poses)
            anchor_idx = 0 
            
            # 💡 消融实验开关：控制是否引入全局参考视角 (Reference View)
            # 你可以从 initialize_pipeline 的参数里传进来
            # use_ref_view = kwargs.get("use_ref_view", True) 
            ...
            for i in range(cam_count):
                ...
                # 🎯 消融点：只有开启时，才加入全局锚点视角 
                if ref:
                    attn_list.append(anchor_idx)
                
                # 🎯 寻找 2 个空间邻居 (维持局部流形一致性)
                dists = []
                curr_e, curr_a = self.camera_poses[i]
                
                for j in range(cam_count):
                    # 排除自己；如果开了 ref，也要排除锚点（因为已经加过了）
                    if i == j or (ref and j == anchor_idx): 
                        continue
                    
                    target_e, target_a = self.camera_poses[j]
                    da = abs(curr_a - target_a)
                    da = da if da <= 180 else 360 - da
                    de = abs(curr_e - target_e)
                    dist = (da**2 + de**2)**0.5
                    dists.append((dist, j))
                
                dists.sort(key=lambda x: x[0])
                for d_val, d_idx in dists[:2]:
                    attn_list.append(d_idx)
                
                self.attention_mask.append(list(set(attn_list)))
        else:
            # === 保留你原来的逻辑 ===
            ...
            #     # self.camera_poses.append((90, 180))
            ...
            #     self.camera_poses.append((45, 90))
            #     self.camera_poses.append((45, 180))
            #     self.camera_poses.append((45, 270))
            #     self.attention_mask.append([front_view_idx, cam_count])
            #     self.attention_mask.append([back_view_idx, cam_count+1])
            #     self.attention_mask.append([front_view_idx, cam_count])
            #     self.attention_mask.append([back_view_idx, cam_count+1])
                ...
                # 尝试正宗的相机
        print('camera_poses', len(self.camera_poses))
    ...
    def __call__(
        ...
        if adain_first != 0:
            ...
            latents = apply_global_style_injection(latents, self.global_mask_96, adain_first)
                        ...
                        if not (use_adjacent_baseline or use_adjacent_segment or use_ref):
                            ...
                            if not yizhi:
                                ...
                                if t < start_step and t > end_step:
                                    ...
                                    yizhi_weight = early_segment_weight * ratio
