...
from src.mul_obj_texture_pipeline import StableSyncMVDPipeline, search_dynamic_cameras, search_dynamic_cameras_submodular
...
def main():
    ...
    if opt.get('dongtai_camera', False):
        # 运行动态搜索算法
        # camera_azims = search_dynamic_cameras(
        #     mesh_path=mesh_path,
        #     mesh_transform={"scale": opt.get('mesh_scale', 2.0)},
        #     num_cameras=opt.get('num_cameras', 8), # 你可以在yaml里设
        #     w1=opt.get('w1', 0.2),
        #     w2=opt.get('w2', 0.6),
        #     w3=opt.get('w3', 0.2),
        #     device=f"cuda:{gpu_id}"
        # )
        camera_azims = search_dynamic_cameras_submodular(
            mesh_path=mesh_path,
            mesh_transform={"scale": opt.get('mesh_scale', 2.0)},
            num_cameras=opt.get('num_cameras', 10), # 你可以在yaml里设
            alpha=0.5,
            device=f"cuda:{gpu_id}"
        )
        # 如果是动态相机，建议关闭 top_cameras，因为搜索算法已经包含了高仰角
        no_top_cameras = True 
    else:
        # 使用 YAML 里的静态配置
        camera_azims = opt.get('camera_azims', [0, 90, 180, 270])
        no_top_cameras = opt.get('no_top_cameras', False)
    # 运行生成过程
    syncmvd(
        prompt=opt['prompt'],
        ...
        dongtai_camera=opt.get('dongtai_camera', False),
        ref=opt.get('ref', False)
    )
